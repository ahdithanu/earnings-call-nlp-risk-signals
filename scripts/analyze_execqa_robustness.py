...
def main() -> None:
    ...
    for scope in SCOPES:
        df[f"z_{scope}"] = (df.groupby("ticker")[f"uncertainty_density_{scope}"]).transform(
    ...
    for scope, label in SCOPES.items():
        ...
        for fe_label, fe in [
            ...
            emit(
                f"  [{fe_label}] coef = {fit.params[f'z_{scope}']:+.3f} pp per 1 SD "
        ...
        for _, g in df.groupby("ticker"):
            if len(g) >= MIN_QUARTERS:
                rs.append(stats.pearsonr(g[f"uncertainty_density_{scope}"], g["growth_w"])[0])
        rs = pd.Series(rs)
        emit(
            f"  per-ticker pearson r (>= {MIN_QUARTERS} quarters, {len(rs)} tickers): "
        ...
        emit(
            f"  lead-lag: predictive r={r_pred:+.3f} (p={p_pred:.3f}) vs "
