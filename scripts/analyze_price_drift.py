...
def main() -> None:
    ...
    emit(
        f"merged calls with both a price outcome and Q&A density: {len(df)} "
        f"({df['ticker'].nunique()} tickers)"
    )

    for outcome, label in [
        ("ret_5d", "5-day post-call reaction"),
        ...
        for spec, fe in [
            ...
            emit(
                f"  {spec}: coef on density_z = {fit.params['density_z']:+.3f} % per 1 SD "
